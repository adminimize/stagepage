...
{#if program}
	<div class="{embedded ? 'border-l-4 border-blue-500 pl-6 ml-4' : ''} program-wrapper">
		<!-- Program Header -->
		<header class="{embedded ? 'mb-6' : 'mb-8'} {embedded ? '' : 'text-center'}">
			<h1 class="{embedded ? 'text-3xl' : 'text-5xl'} font-bold mb-4">
				{#if embedded}
			...
			{#if program.published_date}
				<p class="text-gray-600 mb-4">Published: {new Date(program.published_date).toLocaleDateString()}</p>
			{/if}
				...
				<div class="flex flex-wrap justify-center gap-4 text-sm">
					{#if program.production}
						<span class="bg-blue-100 text-blue-800 px-4 py-2 rounded-full">
							Production: <a href="/productions/{program.production.id}" class="underline font-medium">{program.production.title}</a>
						</span>
					{/if}
					
					{#if program.festival && program.festival.length > 0}
						{#each program.festival as fest}
							<span class="bg-purple-100 text-purple-800 px-4 py-2 rounded-full">
								Festival: <a href="/festivals/{fest.festivals_id?.id || fest.festivals_id}" class="underline font-medium">{fest.festivals_id?.title || 'Festival'}</a>
							</span>
						{/each}
					{/if}
				</div>
					...
					<section class="program-section">
						...
						{#if section.collection === 'programs' && section.content}
							<div class="mb-8 bg-blue-50 rounded-lg p-6 border border-blue-200">
								<h2 class="text-2xl font-bold mb-4 text-blue-800 border-b border-blue-300 pb-2">
								...
								{#if currentDepth < maxDepth}
									{#await loadEmbeddedProgram(section.content.id)}
										<div class="text-center py-8">
											...
											<p class="text-blue-600 mt-2">Loading embedded program...</p>
										</div>
									{:then}
									...
									{/await}
								{:else}
									<div class="text-center py-4 text-gray-600 border border-gray-300 rounded">
										<p class="font-medium">Maximum nesting depth reached</p>
										<a href="/programs/{section.content.id}" class="text-blue-600 hover:underline">
						...
						{:else if section.collection === 'section_credits' && section.content}
							<div class="mb-12">
								<h2 class="text-3xl font-bold mb-8 text-center border-b-2 border-gray-200 pb-4">
									...
									<div class="grid gap-4 {section.content.columns === 1 ? 'grid-cols-1' : section.content.columns === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}">
										{#each program.credits as credit}
											<div class="border rounded-lg p-4 bg-white shadow-sm">
												...
												{#if credit.role}
													<p class="text-gray-600 font-medium">{credit.role.name}</p>
												{/if}
						...
						{:else}
							<div class="mb-8 p-6 bg-gray-50 border border-gray-200 rounded-lg">
								...
								<p class="text-gray-700 text-sm">Section type: {section.collection}</p>
								{#if section.content && section.content.title}
									<p class="text-gray-800 font-medium mt-2">{section.content.title}</p>
								{/if}
							</div>
						{/if}
					</section>
				...
				<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
					{#each program.production.credits as credit}
						<div class="border rounded-lg p-4 bg-white shadow-sm">
							...
							{#if credit.role}
								<p class="text-gray-600 text-sm">{credit.role.name}</p>
							{/if}
						</div>
					{/each}
				</div>
		...
		{#if program.production?.description}
			<div class="mt-8 p-6 bg-gray-50 rounded-lg">
				<h3 class="text-xl font-semibold mb-3">About this Production</h3>
				<p class="text-gray-700 leading-relaxed">{program.production.description}</p>
				
				{#if program.production.venue || program.production.opening_date}
					<div class="mt-4 text-sm text-gray-600">
						{#if program.production.venue}
							<p>📍 {program.production.venue}</p>
						{/if}
						{#if program.production.opening_date && program.production.closing_date}
							<p>📅 {new Date(program.production.opening_date).toLocaleDateString()} - {new Date(program.production.closing_date).toLocaleDateString()}</p>
						{:else if program.production.opening_date}
							<p>📅 {new Date(program.production.opening_date).toLocaleDateString()}</p>
						{/if}
					</div>
				{/if}
			</div>
